        ...
        <div className="page page-with-nav">
            ...
            <div className="scroll-row" style={{ padding: '0 1.25rem', marginBottom: '1rem', gap: '0.5rem' }}>
                {categories.slice(0, 6).map(cat => {
                    ...
                    const catLabel = catId ? (catId.charAt(0).toUpperCase() + catId.slice(1)) : ''
                    return (
                        <button key={catId} className={`filter-chip ${activeFilters.includes(catId) ? 'active' : ''}`} onClick={() => toggleFilter(catId)} id={`search-filter-${catId}`}>
                            {catLabel}
                        </button>
                    )
                })}
            ...
            <div style={{ padding: '0 1.25rem 1.5rem' }}>
                    ...
                    <div style={{ marginBottom: '1.5rem' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                            {menuResults.slice(0, 4).map(item => (
                                <div key={item._id} onClick={() => navigate(`/restaurant/${item.restaurantId}`)} style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', background: 'var(--surface)', borderRadius: 'var(--radius-md)', border: '2px solid var(--border)', padding: '0.75rem', cursor: 'pointer', transition: 'all 0.2s' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                    {results.length > 0 ? results.map(r => <RestaurantCard key={r._id || r.id} restaurant={r} />) : (
